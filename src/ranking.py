...
Usage:
    ...
    python src/ranking.py --candidates data/candidates.jsonl.gz --out output/submission.csv
"""
...
# the job description - core parts used for semantic matching
JD_TEXT = """
...
NOT looking for: pure computer vision, speech, robotics without NLP background.
NOT looking for: consulting-only background (TCS, Infosys, Wipro etc).
Looking for: scrappy product engineers who ship, not just researchers.
"""
...
def compute_semantic_scores(candidates, batch_size=64):
    """
    use sentence-transformers to compare candidate text against JD
...
def build_reasoning(candidate, final_score, features, sem_score):
    ...
    from feature_engineering import MUST_HAVE_SKILLS, _get_full_text
...
def rank_candidates(candidates_path, output_path, top_n=100, use_semantic=True):
    ...
    # combine skill + experience + title + education into one "skills" score
    combined_skill = (
    ...
    # behavioral scores already computed
    ...
    for rank, idx in enumerate(top_idx, start=1):
        ...
        reasoning = build_reasoning(c, score, features, sem)
    ...
    print(f"\nDone. Wrote {len(rows)} candidates to {output_path}")
    print(f"Top candidate: {rows[0]['candidate_id']} (score={rows[0]['score']:.4f})")
    print(f"Reasoning: {rows[0]['reasoning']}")
